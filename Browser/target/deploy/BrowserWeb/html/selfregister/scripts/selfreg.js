function getCustomerNo(thisEvent)
       		 {
       		   if ( validateCustomerNumber( thisEvent ))
       		      {
         			var custNo = window.document.getElementById( "transactionId" ).value;
         			var currentTime = new Date()
           	   		var month = currentTime.getMonth() + 1
	           		var day = currentTime.getDate()
	   	   		var year = currentTime.getFullYear()
                   		var tDay = "" + custNo + year + month + day + "";
          			window.document.getElementById( "userId" ).innerHTML = "Your login Id for internet banking is : " + tDay;
          			window.document.getElementById( "userInfo" ).innerHTML = ".  Please complete and submit the form inorder to activate the User Id."
          		}
          	else
          		{
          			alert("Customer Number must be numeric");
          		}
          
          	}	

function validateCustomerNumber( thisEvent )
{
