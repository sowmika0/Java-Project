		...
		<tr>
			...
			<td>
				...
				<div id="enquiryDataScroller{$formFragmentSuffix}" onscroll="scrollHeading();">
					<xsl:if test="(r/tree) and ($isArc = 'true')">
								...
								<xsl:choose>
									<xsl:when test="(ImDocLoc='SERVER') and not($useCloudImage='YES')">
										<a href="javascript:void(0)">
											...
											<xsl:attribute name="onclick">javascript:fileDownload('<xsl:value-of select="img"/>','<xsl:value-of select="ImDocLoc"/>')</xsl:attribute>
