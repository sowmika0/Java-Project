...
<head>
	...
	<script LANGUAGE="javascript">
		//<![CDATA[
			function setFocus() {
				window.document.forms["UserIdForm"].UserId.focus();
			}
			...
			function doFormBasedAuthentication() {
				var authForm = window.document.forms["AuthForm"];
				authForm.username.value = '<%=session.getAttribute(LoginServlet.USER_ID) %>';
				authForm.password.value = '<%=session.getAttribute(LoginServlet.PASSPHRASE) %>';
				authForm.submit();
			};
...
<body onload="setFocus()">
	...
	<table border="0" width="994">
		...
		<div id = "content">
			<form name="UserIdForm" method="post" target="<%=request.getContextPath()%>/modelbank/unprotected/LoginServlet" AUTOCOMPLETE="OFF">
				<table class="input">
					<tr>
					...
					<tr>
						<td width="23%">
							<input name="UserId" onKeyPress="javascript: return disableEnterKey(event);" size="20"" maxlength="20" />
						</td>
			...
			<p>
				<a href="javascript:postForm(window.document.forms['UserIdForm'])">
