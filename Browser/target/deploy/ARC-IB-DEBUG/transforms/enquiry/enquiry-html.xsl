...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<body topmargin="2" leftmargin="2">
				<xsl:variable name="isHeaderEach">
					<xsl:for-each select="/responseDetails/window/panes/pane/dataSection/atts/att">
						<xsl:if test=". = 'ENQ.HEADERS.ON.EACH.PAGE'">true</xsl:if>
					</xsl:for-each>	
				</xsl:variable>
				<title>
	...
	<xsl:template name="Footer">
		<xsl:choose>
			...
			<xsl:otherwise>
			<!--To extract footer data when  ENQ.HEADERS.ON.EACH.PAGE is not set-->
