...
public class ServiceLocator {
    ...
    public ConnectionFactory lookupJMSConnectionFactory() {
        ...
        try {
            if (_cacheJMSConnectionFactory.containsKey(JMS_CONNECTION_FACTORY_JNDI_NAME)) {
                cxFactory = _cacheJMSConnectionFactory.get(JMS_CONNECTION_FACTORY_JNDI_NAME);
            } else {
                Object factoryObj = _context.lookup(JMS_CONNECTION_FACTORY_JNDI_NAME);
                cxFactory = (ConnectionFactory) factoryObj;
                _cacheJMSConnectionFactory.put(JMS_CONNECTION_FACTORY_JNDI_NAME, cxFactory);
            }
        } catch (NamingException e) {
            // LOGGER.fatal("Configuration error " + e.getMessage(), e);
            throw new RuntimeException("Configuration error [" + JMS_CONNECTION_FACTORY_JNDI_NAME + "] " + e.getMessage());
        }
    ...
    public Destination lookupOFSReplyDestination() {
        ...
        try {
            if (_cacheT24ConnectionFactory.containsKey(T24CONNECTION_FACTORY_JNDI_NAME)) {
                cxFactory = _cacheT24ConnectionFactory.get(T24CONNECTION_FACTORY_JNDI_NAME);
            } else {
            	Object factoryObj = _context.lookup(T24CONNECTION_FACTORY_JNDI_NAME);
                cxFactory = (T24ConnectionFactory) factoryObj;
                _cacheT24ConnectionFactory.put(T24CONNECTION_FACTORY_JNDI_NAME, cxFactory);
            }
        } catch (NamingException e) {
            // LOGGER.fatal("Configuration error " + e.getMessage(), e);
        	 throw new RuntimeException("Configuration error [" + T24CONNECTION_FACTORY_JNDI_NAME + "] " + e.getMessage());
        }
        	 return cxFactory;  
        	 }
