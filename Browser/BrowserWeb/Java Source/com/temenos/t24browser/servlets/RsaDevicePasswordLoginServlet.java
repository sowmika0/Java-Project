 ...
 */
public class RsaDevicePasswordLoginServlet extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
    ...
    /** The logger. */
    private static Logger logger = LoggerFactory.getLogger(RsaDevicePasswordLoginServlet.class);
     ...
     */
    public RsaDevicePasswordLoginServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        logger.debug("Password: " + request.getParameter(AuthenticationConstants.PASSWORD));
        ...
        try {
            if (null != request.getParameter(AuthenticationConstants.USER_ID) && null != request.getParameter(AuthenticationConstants.PASSWORD) && null != request.getParameter(AuthenticationConstants.OTP)) {
                ...
                request.getSession().removeAttribute(AuthenticationConstants.PASSWORD);
                ...
                // The paqssword is added to the session and will be used to log into T24
                // needs to be encrypted first
                CryptographyService cryptoService = CryptographyService.getInstance(config);
                String encPassword = cryptoService.encrypt(request.getParameter(AuthenticationConstants.PASSWORD), true);
    			request.getSession().setAttribute(AuthenticationConstants.PASSWORD, encPassword);
    			request.getSession().setAttribute(AuthenticationConstants.OTP, request.getParameter(AuthenticationConstants.OTP));
                request.getSession().setAttribute(AuthenticationConstants.AUTHENTICATION_TYPE, AuthenticationConstants.PASSWORD);
            	...
            	try {
        	    	...
        	    	authResp = rsaSession.check(request.getParameter(AuthenticationConstants.USER_ID).toUpperCase(),request.getParameter(AuthenticationConstants.OTP).trim());
    			...
    			if (authResponse.getAuthenticationResult() == AuthenticationResponse.ACCESS_OK) {
    				...
    				// We do not need the actual password here ass that is already in the session attributes.
    		        ...
    		        request.getRequestDispatcher(AuthenticationConstants.NEXT_OTP_PASSWORD_PAGE).forward(request, response);
    			} else {
            	...
            	try {
            		...
            		logger.error("Failed with next OTP", e);
