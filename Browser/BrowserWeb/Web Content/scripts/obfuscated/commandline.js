...
function docommand(cmdname)
{
	...
	else if ( ( menuPrefix == "=Y6o" + " ") || ( startChar == "?" ) )
	{
		// Process the menu
		domenu(cmdname);
	}
	...
	else if ( (prefix.substring(0,3) ) == "K5" + " ")
	{
...
function processGeneralCommand(cmdname,browser)
{
		...
		else if ( ( argsNumber == 2 ) && ( F3Check(cmdname) == false ) && ( !validFunction )  && ( !validSpecialFunction ) )
		{
			...
			genForm.ofsOperation.value = "8o1y0";
			buildUtilityRequest("",cmdname,"generalForm","","}rXCtKKy1eth1}6","","250","250","");	
		}
		...
		{
			buildUtilityRequest("}XC6Y5C0Yty",cmdname,"generalForm","","","","","","");
		}
...
function processList(ftn,key,app,browser)
{
		...
		else if ( ftn == "L" )
		{
			...
			{
				// Get enquiry results using the full selection criteria
				doenqDropfieldList( "Y6l" + " %" + app, dropfield );
			}
		}
...
function dotab(cmdname)
{
	...
	buildUtilityRequest("}XC>YhCht8Crbt=YX", cmdname,"generalForm","","","","","","");
...
function doworkspace(cmdname)
{
	...
	buildUtilityRequest("}XC>YhC5}bPXKteYCq=y", cmdname.substring(4,99),"generalForm",target,"","","","","");
...
function doapi(cmdname)
{
	...
	if (processAPIString(cmdname) == 2){	
		buildUtilityRequest("","","generalForm","","tK1CbYloYXh",cmdname,"","","");
	}
}
...
function doenqRun(cmdname)
{
	...
	var theEnqName = cmdname.substring(6, cmdname.length);
 	...
 	if ( queryPrefix == "loYba" + " ")
 	{
		...
		doEnquiryRequest("XYyYeh1}6", cmdname, dropfield, "", "", "", "Y6l" + " " + theEnqName, "generalForm", "01XKyta");
	}
	else
	{
		doEnquiryRequest("bo6", cmdname, dropfield, "", "", "", "Y6l" + " " + theEnqName, "generalForm", "01XKyta");
	}
}
/**
...
function doenqList(cmdname)
{
	...
	doEnquiryRequest("bo6", cmdname, dropfield, "", "", "", "NEW", "generalForm", "01XKyta");
}
 ...
 function doenqSearch(cmdname)
{
	...
	doEnquiryRequest("XYyYeh1}6", cmdname, dropfield, "", "", "", "NEW", "generalForm", "01XKyta");
}
...
{
	doEnquiryRequest(action, enqname, "transactionId", "", "", "", "NEW", "generalForm", "01XKyta");
}
...
function docompositescreen(cmdname){
	...
	buildUtilityRequest("}XC>YhCe}=K}X1hYCXebYY6Cq=y",cmdname,"generalForm","","","","","","");
...
function domenu(cmdname)
{
	...
	if (menuTarget)
	{
		buildUtilityRequest("}XC>YhC=Y6oCq=y",menuCmd,"generalForm",menuTarget,"","","","","");
	}
	else
	{
		buildUtilityRequest("}XC>YhC=Y6oCq=y",menuCmd,"generalForm","","","","","","");
	}
}
...
{
	buildUtilityRequest("}XCbo6CK5CKb}eYXX",cmdname,"generalForm","","","","","","");
}
	...
	if ( ( target != "" ) && ( target != "NEW" ) )
	{
		...
		if ( appreqForm != undefined )
		{
			...
			if ( targetApp && targetKey )
			{
				...
				appreqForm.unlock.value = "6}Co6y}eP";
			}
		}
	}
}
