...
if(confirm('The Adobe SVG Viewer is not installed. Download now?'))
	{
		//Need to call another page to set the browser history for IE.
		//This is because once we install the svg plugin, the Adobe website calls history.back() and
		//if we do not set the browser history we get an expired webpage.
		var contextRoot = getWebServerContextRoot();
		var url = location.protocol + "//" + location.host + "/" + contextRoot + "/jsps/svgInstall.jsp";
