...
public class AESRSACryptoService {
     ...
     * 
     * @param config - contains the information read from the config file
     */
    public AESRSACryptoService(AuthenticationServerConfiguration config){
         ...
         keyAlgorithm = config.getConfigValue(AuthenticationServerConfiguration.CRYPTO_ALGORITHM);
         cryptoProvider = config.getConfigValue(AuthenticationServerConfiguration.CRYPTO_PROVIDER);
         //Key path for generating the public key to decrypt the Digest
         publicKeyPath = config.getConfigValue(AuthenticationServerConfiguration.SIGNATURE_KEY_PATH);
         signatureAlgorithm = config.getConfigValue(AuthenticationServerConfiguration.SIGNATURE_ALGORITHM);
         logger.debug("keyPassword =" +keyPassword +" alias ="+ alias + " keystorePassword ="+keystorePassword + " keystorePath ="+keystorePath); 
         logger.debug("keyalgorithm =" +keyAlgorithm + " publicKeyPath ="+ publicKeyPath + " signatureAlgorithm =" + signatureAlgorithm);
	...
	public PublicKey generatePublicKey() throws IOException,NoSuchAlgorithmException, InvalidKeySpecException, NoSuchProviderException	{
	    ...
	    logger.debug("Public Key Spec Created");
	 ...
	 * @param inpBytes
	 * @param key
	 * @param xform - to specify the algorithm
	 * @return
	 * @throws NoSuchAlgorithmException
	...
	public byte[] rsaDecrypt(byte[] inpBytes, Key key, String type) throws
		NoSuchAlgorithmException, InvalidKeyException, IllegalBlockSizeException,
		NoSuchPaddingException, BadPaddingException, InvalidAlgorithmParameterException, ClassNotFoundException, InstantiationException, IllegalAccessException{
		Cipher cipher = null;
		Provider provider = null;
		if(cryptoProvider!=null && !cryptoProvider.equals("")){
			Class providerClass = Class.forName(cryptoProvider); //Throws ClassNotFoundException
			provider = (Provider) providerClass.newInstance(); //Throws InstantiationException
			Security.addProvider(provider);
		}
		//Deciding the algorithm based the data if digest PKCS and for key OAEP
		if (type.equalsIgnoreCase("Digest")){
			cipher = provider!=null?Cipher.getInstance(signatureAlgorithm,provider):Cipher.getInstance(signatureAlgorithm);
		}else if(type.equalsIgnoreCase("Key")){
			cipher = provider!=null?Cipher.getInstance(keyAlgorithm,provider):Cipher.getInstance(keyAlgorithm);
		}
	...
	public PrivateKey genPrvKeyFromStore() throws KeyStoreException, NoSuchAlgorithmException, UnrecoverableKeyException, CertificateException, IOException {
		...
		key = store.getKey(alias, keyPassword.toCharArray());
